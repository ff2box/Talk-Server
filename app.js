  ...
  .post( async (req, res, next) => {
    ...
    if (sessions[req.cookies.sessionId] == req.body.captcha.toLowerCase()) {
      ...
      if (user) {
        ...
        res.jsonp({status: 403, data: {message :'登入名、密码错啦, 再蒙一下 ~_~ '}})
      }
      
    } else {
      res.jsonp({status: 403, data: {message :'验证码又输错啦, 一定是姿势不对 #_#'}})
...
app.route('/api/add-post')
  ...
  app.get('/api/delete-comment/:commentid', async (req, res, next) => {
    ...
    if (req.user.id == comment.userId) {
      ...
      res.redirect('/api/post/' + comment.postId)
    } else {
  ...
  app.route('/api/send-email')
    ...
    .post( async (req, res, next) => {
      ...
      if (user) {
        ...
        res.jsonp({status: 200, data: {message: '邮件发送成功，记得回邮箱确认信息 @_@'}})
      } else {
        // res.render('page-404.pug', {data: '用户名或邮箱错误'})
        res.jsonp({status: 403, data: {message: '用户名或邮箱错误'}})
      }
    })
  // 重置密码
